...
class BookKeeper:
    ...
    def calculate_transaction_costs(self, amount:float, symbol: str, month: Month, year: Year) -> tuple[float, float] | None:
        if not self._data_aggregator.is_stock_price_available(symbol, month, year):
            return None

        if self._balance <= TRANSACTION_FEE:
            return None
    ...
    def calculate_current_position(self, company_symbol: str, month: Month, year: Year) -> float:
        position = 0
        ...
        assert position >= 0, "Cannot have negative position!"
    ...
    @property
    def invested_companies(self) -> int:
